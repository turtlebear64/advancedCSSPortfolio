...
const getPosts = () => {
  fs.readdir(dirPath, (err, files) => {
    if (err) {
      return console.log('No content was found: ' + err);
    }
    let ilist = [];
    files.forEach((file, i) => {
      ...
      fs.readFile(`${dirPath}/${file}`, 'utf8', (err, contents) => {
        ...
        post = {
          id: timestamp,
          title: metadata.title ? metadata.title : 'Title Error',
          author: metadata.author ? metadata.author : 'Author Error',
          date: publishedDate ? publishedDate : 'Date Error',
          time: parsedDate['time'],
          thumbnail: metadata.thumbnail,
          content: content ? content : 'Content Error',
          slug: metadata.slug ? metadata.slug : '404 Error',
